    ...
    def guardar(self):
        ...
        try:
            if not self.tipo_cambio:
                # Verificar que no exista para esa fecha
                existe = self.session.query(TipoCambio).filter_by(fecha=fecha).first()
                if existe:
                    QMessageBox.warning(
                        self,
                        "Error",
                        f"Ya existe tipo de cambio para {fecha.strftime('%d/%m/%Y')}"
                    )
                    return
...
class TipoCambioWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.tabla.setHorizontalHeaderLabels([
            "Fecha", "Precio Compra", "Precio Venta", "Acciones"
        ])
    ...
    def mostrar_tipos_cambio(self, tipos_cambio):
        ...
        # Actualizar contador
        self.lbl_contador.setText(f"📊 Total: {len(tipos_cambio)} registro(s)")
    ...
    def eliminar_tipo_cambio(self, tipo_cambio):
        ...
        respuesta = QMessageBox.question(
            ...
            f"¿Eliminar tipo de cambio del {tipo_cambio.fecha.strftime('%d/%m/%Y')}?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        
        if respuesta == QMessageBox.StandardButton.Yes:
            try:
                self.session.delete(tipo_cambio)
                self.session.commit()
                QMessageBox.information(self, "Éxito", "Tipo de cambio eliminado")
                self.cargar_tipos_cambio()
            except Exception as e:
                self.session.rollback()
    ...
    def descargar_plantilla(self):
        ...
        try:
            ...
            ws.append(['2024-01-16', 3.860, 3.880])
            ws.append(['2024-01-17', 3.840, 3.860])
    ...
    def importar_excel(self):
        ...
        try:
            ...
            for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    if isinstance(row[0], datetime):
                        ...
                        fecha = datetime.strptime(str(row[0]), '%Y-%m-%d').date()
            ...
            QMessageBox.information(self, "Importación", mensaje)
            self.cargar_tipos_cambio()
