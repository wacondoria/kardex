...
class ProveedoresWindow(QWidget):
    ...
    def init_ui(self):
        ...
        btn_importar.setStyleSheet("""
            QPushButton {
                background-color: #107C41; /* Lighter Excel Green */
                color: white;
                padding: 10px 20px;
                border: none;
                border-radius: 5px;
    ...
    def descargar_plantilla(self):
        """Crea y guarda una plantilla de Excel para la importación"""
        ...
        try:
            ...
            ws.column_dimensions['C'].width = 40  # Dirección
            ws.column_dimensions['D'].width = 20  # Teléfono
            ws.column_dimensions['E'].width = 30  # Email
            ws.column_dimensions['F'].width = 30  # Contacto
    ...
    def importar_excel(self):
        ...
        try:
            ...
            for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                ...
                contacto = str(row[5]).strip() if row[5] else None

                # Validación básica
                if not ruc and not razon_social:
                    # Si ambos están vacíos, probablemente es una fila en blanco, la saltamos
                    continue
            ...
            if not proveedores_excel and not errores_lectura:
                 QMessageBox.warning(self, "Archivo Vacío", "No se encontraron datos de proveedores válidos en el archivo.")
                 return

            # 3. Procesar datos (Crear vs Actualizar)
            # Para eficiencia, buscamos todos los RUCs existentes en la BD de una sola vez
            ...
            for data in proveedores_excel:
                ...
                if ruc in mapa_existentes:
                    ...
                    prov.activo = True # Reactivar si estaba inactivo
                    actualizados += 1
                else:
                    # Crear
                    prov = Proveedor(
            ...
            self.cargar_proveedores() # Recargar la tabla
