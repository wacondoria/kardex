...
def verificar_y_actualizar_db(db_url='sqlite:///kardex.db'):
    ...
    try:
        ...
        # Verificar columna 'fecha_registro_contable' en 'ventas'
        if inspector.has_table('ventas'):
             columns_ventas = [col['name'] for col in inspector.get_columns('ventas')]

             if 'fecha_registro_contable' not in columns_ventas:
                print("⚠️  Actualizando 'ventas': Agregando 'fecha_registro_contable'...")
                with engine.connect() as connection:
                    connection.execute(text("ALTER TABLE ventas ADD COLUMN fecha_registro_contable DATE"))
                    connection.execute(text("UPDATE ventas SET fecha_registro_contable = fecha WHERE fecha_registro_contable IS NULL"))
                    connection.commit()
                print("✓  Columna 'fecha_registro_contable' añadida a 'ventas'.")

             # Verificar columna 'numero_documento' en 'ventas'
             if 'numero_documento' not in columns_ventas:
                 print("⚠️  Actualizando 'ventas': Migrando a 'numero_documento'...")
                 with engine.connect() as connection:
                     connection.execute(text("ALTER TABLE ventas ADD COLUMN numero_documento VARCHAR(20)"))

                     if 'serie_documento' in columns_ventas and 'correlativo_documento' in columns_ventas:
                         print("ℹ️  Combinando serie y correlativo...")
                         connection.execute(text("UPDATE ventas SET numero_documento = serie_documento || '-' || correlativo_documento"))
                     else:
                         # Fallback si no existen las columnas antiguas, aunque sea raro
                         print("ℹ️  No hay columnas antiguas, seteando default...")
                         connection.execute(text("UPDATE ventas SET numero_documento = 'UNKNOWN' WHERE numero_documento IS NULL"))

                     connection.commit()
                 print("✓  Columna 'numero_documento' añadida y poblada en 'ventas'.")
